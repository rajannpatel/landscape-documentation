...
The best way is to use [upgrade profiles](/docs/explanation/terms/profiles/upgrade-profile), which rely on [access groups](/docs/explanation/terms/access-groups). If an access group is already set up for the group of machines you want to keep updated automatically, click on its name. If not, you must create an access group for them:
...
Manage them via a [package profile](/docs/explanation/terms/profiles/package-profile).
